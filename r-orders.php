...
<h1 class="cat">Hello <span class="cat red"><?php echo $_SESSION['restaurant_name']; ?></span><br>Here are your Recent Orders</h1>
<div class="container">
	<?php
		...
		dish.name, dish.image, dish.price, dish.restaurant_name, dish.restaurant_id
		FROM (( orders
		INNER JOIN customer ON orders.user_id = customer.id)
		INNER JOIN dish ON orders.dish_id = dish.id) where dish.restaurant_id=$r_id order by orders.id DESC";
