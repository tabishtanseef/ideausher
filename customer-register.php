...
if (isset($_POST['submit'])) {
	...
	if (!$error) {		 
		$g = "SELECT id FROM customer WHERE num ='$num'";
		...
		if (strlen($ch['id']) != 0)
		{
		  echo"<script>alert('This Account is already registered!');</script>";
			...
			if(mysqli_query($conn, "INSERT INTO customer(name,num,email,password,address,city,veg,nonveg) VALUES
			('$name','$num','$email','$password','$add','$city',$veg, $nonveg)")) {
				header("Location: index.php");
			} else {
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="css/c-register.css" />
	 <!-- Custom styles -->
    <style>
	</style>
  </head>
		...
		<div class="container">
			<div class="row">
				<div class="col-md-6 fo" >
					<div class="card">
					  <div class="card-body">
						<h5 class="card-title">Register Yourself with <a href="index.php" class="food">FoodShala</a></h5>
