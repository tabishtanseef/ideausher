...
<div class="container">
	...
	<?php
		if(isset($_GET['r_id'])){
			...
			echo "<h1 class='cat'>Best Dishes From $r_name</h1>";
			
			$get_attendance="select * from dish where restaurant_id='$r_id' order by rand()";
			$run_attendance= mysqli_query($conn, $get_attendance);
			$n=1;
			while($row_attendance=mysqli_fetch_array($run_attendance))
			{
			...
			echo "<h1 class='cat'>Choose from our Best Dishes</h1>";
			$get_attendance="select * from dish order by rand()";
			$run_attendance= mysqli_query($conn, $get_attendance);
			$n=1;
			while($row_attendance=mysqli_fetch_array($run_attendance))
			{
				...
				echo "
				<div class='col-lg-3 col-6 pop' id='cont'>
				<div class='card'>
				  <div class='bg-image hover-overlay ripple' data-ripple-color='light'>
					<img src='img/dish/$image' class='img-fluid' />
					<div class='mask' style='background-color: rgba(251, 251, 251, 0.15)'></div>
